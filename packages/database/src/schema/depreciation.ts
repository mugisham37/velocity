...
export const depreciationSchedules = pgTable(
  'depreciation_schedules',
  {
    ...
    depreciationMethod: varchar('depreciation_method', { length: 50 }).notNull(), // Straight Line, Declining Balance, Units of Production, Sum of Years Digits
    ...
    depreciableAmount: decimal('depreciable_amount', { precision: 15, scale: 2 }).notNull(),
    ...
    // Schedule Dates
    startDate: timestamp('start_date').no
  ...
  (table) => ({
    scheduleNumberCompanyIdx: unique().on(table.scheduleNumber, table.companyId),
...
export const depreciationEntries = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    scheduleId: uuid('schedule_id').references(() => depreciationSchedules.id).notNull(),
    ...
    // Tax Depreciation
    taxDepreciationAmount: decimal('tax_depreciation_amount', { precision: 15, scale: 2 }),
    taxAccumulatedDepreciation: decimal('tax_accumulated_depreciation', { precision: 15, scale: 2 }),
...
export const assetRevaluations = pgTable(
  'asset_revaluations',
  {
    ...
    previousBookValue: decimal('previous_book_value', { precision: 15, scale: 2 }).notNull(),
    newFairValue: decimal('new_fair_value', { precision: 15, scale: 2 }).notNull(),
    revaluationSurplus: decimal('revaluation_surplus', { precision: 15, scale: 2 }).notNull(),
  ...
  (table) => ({
    revaluationNumberCompanyIdx: unique().on(table.revaluationNumber, table.companyId),
    assetIdx: index().on(table.assetId),
...
// Relations
export const depreciationSchedulesRelations = relations(depreciationSchedules, ({ one, many }) => ({
...
export const assetRevaluationsRelations = relations(assetRevaluations, ({ one }) => ({
  company: one(companies, {
...
export const depreciationMethodsRelations = relations(depreciationMethods, ({ one }) => ({
  company: one(companies, {
