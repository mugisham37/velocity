...
export const vendorPerformanceMetrics = pgTable('vendor_performance_metrics', {
  ...
  un('unit', { length: 20 }), // %, days, score, etc.
...
export const vendorCategoryMemberships = pgTable('vendor_category_memberships', {
  id: uuid('id').primaryKey().defaultRandom(),
  vendorId: uuid('vendor_id').references(() => vendors.id).notNull(),
  categoryId: uuid('category_id').references(() => vendorCategories.id).notNull(),
  isPrimary: boolean('is_primary').default(false).notNull(),
  companyId: uuid('company_id').references(() => companies.id).notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
...
export const vendorCategoriesRelations = relations(vendorCategories, ({ one, many }) => ({
...
export const vendorCategoryMembershipsRelations = relations(vendorCategoryMemberships, ({ one }) => ({
  vendor: one(vendors, {
...
export const vendorPortalUsersRelations = relations(vendorPortalUsers, ({ one }) => ({
  vendor: one(vendors, {
