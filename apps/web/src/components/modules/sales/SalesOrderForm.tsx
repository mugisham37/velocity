...
const salesOrderTaxSchema = z.object({
...
const salesOrderSchema = z.object({
  naming_series: z.string().default('SAL-ORD-.YYYY.-'),
  ...
  taxes: z.array(salesOrderTaxSchema).default([]),
...
export default function SalesOrderForm({
    ...
    const loadCustomers = async () => {
      try {
        const response = await getList('Customer', {
          fields: ['name', 'customer_name', 'customer_type', 'default_currency', 'selling_price_list'],
          limit: 100,
        });
      ...
      const loadCustomerDetails = async () => {
        try {
          ...
          if (customer.default_currency) {
            ...
            form.setValue('price_list_currency', customer.default_currency);
          }
          if (customer.default_price_list) {
            form.setValue('selling_price_list', customer.default_price_list);
          }
  ...
  const handleSave = async (data: SalesOrderFormData) => {
    try {
      if (onSave) {
        await onSave(data);
  ...
  const formSections = [
    {
      ...
      collapsible: false,
      fields: [
        'naming_series', 'customer', 'customer_name', 'order_type',
        'transaction_date', 'delivery_date', 'customer_po', 'po_date'
      ]
    },
    {
    ...
    },
    {
      ...
      collapsible: true,
      fields: [
        'company', 'cost_center', 'project', 'source', 'campaign',
        'sales_partner', 'commission_rate', 'payment_terms_template'
      ]
    },
    {
...
function getFieldConfig(fieldName: string) {
  const configs: Record<string, any> = {
    ...
    selling_price_list: { label: 'Price List', type: 'text' },
    ...
    sales_partner: { label: 'Sales Partner', type: 'text' },
