...
export class FinancialRatiosService {
  ...
  async generateFinancialRatios(
    ...
    try {
      ...
      const profitabilityRatios =
        this.calculateProfitabilityRatios(financialData);
      ...
      this.logger.error('Failed to generate financial ratios', {
        error,
        input,
        companyId,
      });
  ...
  private extractFinancialData(balanceSheet: any, profitLoss: any) {
    ...
    const currentAssets =
      this.findLineAmount(balanceSheet.lines, 'current assets') || 0;
    const totalAssets = balanceSheet.totalAssets || 0;
    const currentLiabilities =
      this.findLineAmount(balanceSheet.lines, 'current liabilities') || 0;
    ...
    const accountsReceivable =
      this.findLineAmount(balanceSheet.lines, 'receivable') || 0;
    ...
    const grossProfit =
      this.findLineAmount(profitLoss.lines, 'gross profit') || revenue * 0.3; // Estimate
    const operatingIncome =
      this.findLineAmount(profitLoss.lines, 'operating income') ||
      netIncome * 1.2; // Estimate
    const interestExpense =
      this.findLineAmount(profitLoss.lines, 'interest') || 0;
    const costOfGoodsSold =
      this.findLineAmount(profitLoss.lines, 'cost of goods sold') ||
      revenue * 0.7; // Estimate
  ...
  private calculateLiquidityRatios(data: any): FinancialRatio[] {
    ...
    const quickRatio =
      data.currentLiabilities !== 0 ? quickAssets / data.currentLiabilities : 0;
    ...
    // Cash Ratio
    const cashRatio =
      data.currentLiabilities !== 0 ? data.cash / data.currentLiabilities : 0;
    ...
    ratios.push({
      ...
      interpretation:
        cashRatio >= 0.2 ? 'Strong cash position' : 'Limited cash reserves',
  ...
  private calculateProfitabilityRatios(data: any): FinancialRatio[] {
    ...
    const netProfitMargin =
      data.revenue !== 0 ? (data.netIncome / data.revenue) * 100 : 0;
    ...
    const roa =
      data.totalAssets !== 0 ? (data.netIncome / data.totalAssets) * 100 : 0;
    ...
    const roe =
      data.totalEquity !== 0 ? (data.netIncome / data.totalEquity) * 100 : 0;
  ...
  private calculateLeverageRatios(data: any): FinancialRatio[] {
    ...
    const debtToEquity =
      data.totalEquity !== 0 ? data.totalLiabilities / data.totalEquity : 0;
  ...
  private calculateEfficiencyRatios(data: any): FinancialRatio[] {
    ...
    // Asset Turnover
    const assetTurnover =
      data.totalAssets !== 0 ? data.revenue / data.totalAssets : 0;
    ...
    const inventoryTurnover =
      data.inventory !== 0 ? data.costOfGoodsSold / data.inventory : 0;
