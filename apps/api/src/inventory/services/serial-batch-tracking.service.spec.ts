...
describe('SerialBatchTrackingService', () => {
  ...
  describe('createSerialNumber', () => {
    it('should create a serial number successfully', async () => {
      const createDto: CreateSerialNumberDto = {
        serialNumber: 'SN001',
        itemId: 'item-1',
    ...
    it('should throw error if serial number already exists', async () => {
      ...
      await expect(
        service.createSerialNumber(createDto, 'company-1', 'user-1')
  ...
  describe('createBatchNumber', () => {
    it('should create a batch number successfully', async () => {
      const createDto: CreateBatchNumberDto = {
  ...
  describe('getBatchNumbers', () => {
    it('should return batch numbers with filters', async () => {
      const mockBatchNumbers = [
        {
          id: 'batch-1',
          batchNumber: 'BATCH001',
          itemId: 'item-1',
      ...
      mockDb.orderBy.mockReturnValueOnce(mockDb);
      mockDb.limit.mockReturnValueOnce(mockDb);
      mockDb.offset.mockResolvedValueOnce(mockBatchNumbers);

      const result = await service.getBatchNumbers(
        { itemId: 'item-1' },
        'company-1',
        50,
        0
      ...
      expect(result).toEqual(mockBatchNumbers);
    });
  });
  ...
  describe('getTraceabilityReport', () => {
    it('should return traceability report', async () => {
      ...
      const result = await service.getTraceabilityReport(query, 'company-1');
